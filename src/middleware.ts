...
async function getRegionMap(cacheId: string) {
  ...
  // If no backend URL, use a fallback region map for the booking-only mode
  if (!BACKEND_URL) {
    if (!regionMap.has(DEFAULT_REGION)) {
      // Create a minimal fallback region for booking-only mode
  ...
  if (
    ...
    try {
      ...
      // If backend fetch fails, use fallback region for booking-only mode
      if (!regionMap.has(DEFAULT_REGION)) {
        const fallbackRegion: HttpTypes.StoreRegion = {
          id: "fallback",
          name: "Default",
